...
object BarcodeInfoUtils {
    ...
    fun BarcodeInfo.broadcast(context: Context) {
        val intent = Intent(ACTION_DECODE_DATA)
        intent.putExtra("source_byte", this.sourceData)
        intent.putExtra("barcode_string", this.formatData)
        intent.putExtra("decode_time", this.decodeTime)
        intent.putExtra("aim_string", this.aim)
        context.sendBroadcast(intent)
    }

    fun BarcodeInfo.simulate(context: Context, settings: Settings) {
    ...
    private const val ACTION_DECODE_DATA = "com.action.DECODE_DATA"
